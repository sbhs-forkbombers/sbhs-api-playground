...
function getSHSAPI(path, token, params, post, cb) {
	...
	if (!post) {
		req = request('http://student.sbhs.net.au' + path + '?' + qs.stringify(params), onResponse);
	} else {
		req = request.post({
...
module.exports = {
	'get': function request(name, opts, sessID, cb) { // get the API name using sessID and opts
		'use strict';
	...
	'isAPI': function isAPI(name) { // true if there's an API called name
		'use strict';
