...
module.exports = {
	'getAuthCode': function(res, SESSID) { // get authorisation from the user
		'use strict';
		httpHeader(res,
	...
	'getAuthToken': function(res, uri, cb) { // exchange the authCode for an authorisation token
		'use strict';
	...
	'refreshAuthToken': function(refreshToken, sessID, cb) { // refresh the access token using the refresh token
		'use strict';
	...
	'refreshTokenIfNeeded': function(sessID, cb) { // call cb() after it's ensured that an access token exists.
		'use strict';
		var session = global.sessions[sessID];
